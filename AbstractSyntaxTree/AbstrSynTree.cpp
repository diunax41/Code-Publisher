...
//----< add html tag <span> to lines >--------------------

void ASTNode::add_span(std::vector<std::string>& lines)
{
	size_t start = startLineCount_ - 1;
	size_t end = endLineCount_ - 1;
	if (type_ == "class")
	{
		lines.at(start).insert(0, "<span class=\"toggleClassDisplay\">");
		lines.at(end).append("</span>");
	}
	else if (type_ == "function")
	{
		if (startLineCount_ == endLineCount_)
		{
			size_t pos = lines.at(start).find(')');
			lines.at(start).insert(pos + 1, "<span class=\"toggleMethodDisplay\">");
			lines.at(end).append("</span>");
		}
		else
		{
			lines.at(start).insert(0, "<span class=\"toggleMethodDisplay\">");
			lines.at(end).append("</span>");
		}
	}
}
